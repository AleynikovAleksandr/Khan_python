...
BEGIN
    ...
    IF NOT EXISTS (SELECT 1 FROM Ingredient WHERE ingredient_id = ingredientID) THEN
        SIGNAL SQLSTATE '45000'
        SET MESSAGE_TEXT = 'Такого ингредиента не существует.';
    END IF;
    ...
    -- Если ингредиент связан с блюдами, запрет на удаление
    IF ingredientCount > 0 THEN
        SIGNAL SQLSTATE '45000'
        SET MESSAGE_TEXT = 'Выбранный ингредиент невозможно удалить, так как к нему привязано блюдо.';
    ELSE
